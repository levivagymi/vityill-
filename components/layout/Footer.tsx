      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          <div className="footer-col lg:col-span-1">
            <Link href={`/${lang}`} className="inline-block mb-4" data-cursor="view">
              <div className="font-heading text-2xl font-semibold text-foreground">Vityilló</div>
              <div className="text-xs uppercase tracking-[0.25em] text-foreground/35 font-sans mt-0.5">Vendégház</div>
            </Link>
            <p className="text-sm text-foreground/45 font-sans leading-relaxed mt-3 max-w-xs">
              {dict.footer.tagline}
            </p>
          ...
          <div className="footer-col">
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-3 text-sm text-foreground/55 font-sans">
                ...
                <a href={`tel:${dict.footer.phone}`} className="hover:text-foreground transition-colors">
                  {dict.footer.phone}
                </a>
              </li>
              <li className="flex items-center gap-3 text-sm text-foreground/55 font-sans">
          ...
          <div className="footer-col">
            ...
            <h4 className="font-sans text-xs uppercase tracking-[0.2em] text-foreground/25 mb-3">
              Nyelv / Language
            </h4>
            <div className="flex gap-2">
              {(['hu', 'en', 'de'] as Locale[]).map((l) => (
                <Link
                  key={l}
                  href={`/${l}`}
                  className={`text-xs py-1.5 px-3 rounded-full uppercase tracking-wider font-sans transition-all ${
                    lang === l
                      ? 'bg-foreground text-background font-bold'
                      : 'bg-foreground/[0.06] text-foreground/45 hover:text-foreground/80 border border-foreground/[0.08]'
                  }`}
                >
                  {l}
                </Link>
        ...
        <div className="mt-12 pt-6 border-t border-foreground/[0.06] flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-foreground/[0.22] font-sans">
            © {new Date().getFullYear()} Vityilló Vendégház. {dict.footer.rights}
          </p>
          <div className="flex gap-4">
            <a href="#" className="text-xs text-foreground/[0.22] hover:text-foreground/50 font-sans transition-colors">
              {dict.footer.privacy}
            </a>
            <a href="#" className="text-xs text-foreground/[0.22] hover:text-foreground/50 font-sans transition-colors">
              {dict.footer.terms}
